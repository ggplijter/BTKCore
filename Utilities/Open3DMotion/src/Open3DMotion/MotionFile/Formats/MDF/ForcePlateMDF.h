	...
	class ForcePlateMDF : public ForcePlate
  {
    ...
    const char* MDFChannelUnits(size_t ichannel) const;
		...
		*/
		...
		UInt8 MDFPlateID() const noexcept(false);
...
*/
    ...
    void DefaultOutline();
		...
		static const char* IDtoModel(UInt8 id) noexcept(false);

		static const char* IDtoType(UInt8 id) noexcept(false);
	};
}
