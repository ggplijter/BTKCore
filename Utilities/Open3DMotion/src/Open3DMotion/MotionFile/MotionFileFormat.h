...
namespace Open3DMotion
{
  ...
  public:
    ...
    virtual bool Probe(const MotionFileHandler& context, TreeValue*& readoptions, std::istream& is) const noexcept(false) = 0;

		// Read the file into trial object (or derived trial object)
    virtual TreeValue* Read(const MotionFileHandler& context, std::istream& is, const BinMemFactory& memfactory, const TreeValue* readoptions) const noexcept(false) = 0;

		// Write the file
    virtual void Write(const MotionFileHandler& context, const TreeValue* contents, std::ostream& os, const TreeValue* writeoptions) const noexcept(false) = 0;
